...
function Assert-UpstreamDataset(
    ...
    foreach ($path in @($DatasetPath, $ManifestPath)) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "缺少 MODEL-002 上游输入：$path"
        }
    }
    & cargo run --quiet --locked --bin validate_dataset_manifest -- $ManifestPath
    if ($LASTEXITCODE -ne 0) {
        throw "MODEL-002 上游 Dataset Manifest v1 Rust 校验失败：$ManifestPath"
    }
    ...
    if ([string]$manifest.dataset.name -ne $ExpectedDatasetName -or
        ...
        throw "MODEL-002 上游 dataset 名称、路径或 SHA-256 与 manifest 不一致。"
    }
    return [ordered]@{
...
if ($LASTEXITCODE -ne 0 -or $gitCommit -notmatch '^[0-9a-f]{40,64}$') {
    throw "无法读取 MODEL-002 生成时 Git commit。"
}
if ([string]::IsNullOrWhiteSpace($Version)) {
    $Version = "{0}.{1}.model002" -f (Get-Date).ToUniversalTime().ToString("yyyy-MM-dd"), $gitCommit.Substring(0, 7)
}
if ($Version -notmatch '^[A-Za-z0-9._-]+$') {
    throw "Version 只能包含 ASCII 字母、数字、点、下划线和连字符。"
}

$artifactDirectory = Join-Path $ArtifactRoot "models/elo-baseline/$Version"
if (Test-Path -LiteralPath $artifactDirectory) {
    throw "artifact version 已存在，禁止覆盖：$artifactDirectory"
}
$pythonEntrypoint = Join-Path $repositoryRoot "research/model002_elo_baseline.py"
$temporaryArtifactOne = Join-Path ([System.IO.Path]::GetTempPath()) ("prob-scout-model002-{0}.json" -f [guid]::NewGuid().ToString("N"))
$temporaryArtifactTwo = Join-Path ([System.IO.Path]::GetTempPath()) ("prob-scout-model002-{0}.json" -f [guid]::NewGuid().ToString("N"))
try {
    foreach ($temporaryArtifact in @($temporaryArtifactOne, $temporaryArtifactTwo)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "MODEL-002 Python artifact 构建失败。"
        }
    }
    $firstHash = Get-Sha256 $temporaryArtifactOne
    $secondHash = Get-Sha256 $temporaryArtifactTwo
    if ($firstHash -ne $secondHash) {
        throw "MODEL-002 相同输入重放得到不同 artifact：first=$firstHash, second=$secondHash"
    }
    New-Item -ItemType Directory -Path $artifactDirectory | Out-Null
    $artifactPath = Join-Path $artifactDirectory "elo-baseline-artifact.json"
...
if ([int]$artifact.artifact_schema_version -ne 1 -or
    [string]$artifact.model.family -ne "elo_baseline" -or
    [string]$artifact.model.strategy -ne "global_chronological_elo" -or
    [string]$artifact.final_test_evaluation.status -ne "sealed_not_evaluated") {
    throw "MODEL-002 artifact 合同校验失败。"
}
if ($artifact.final_test_evaluation.PSObject.Properties.Name -contains "series_ids") {
    throw "MODEL-002 artifact 不得包含 final test IDs。"
}
...
$diffHash = $null
if ($dirty) {
    $statePath = Join-Path ([System.IO.Path]::GetTempPath()) ("prob-scout-model002-worktree-{0}.txt" -f [guid]::NewGuid().ToString("N"))
    try {
...
$artifactManifest = [ordered]@{
    ...
    generator = [ordered]@{
        entrypoint = "research/build_elo_baseline.ps1"
...
[pscustomobject]@{
    ...
    DevelopmentSeries = [int]$artifact.development_summary.series_count
    UniqueTeams = [int]$artifact.development_summary.unique_team_count
